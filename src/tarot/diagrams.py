...
"""Reliability diagrams as self-contained SVG. `docs/07`: "Publish these."
    ...
    svg = reliability_diagram(ece.equal_mass, title="Tarot-Draw 4B — published")
    Path("reliability.svg").write_text(svg)

No plotting dependency, matching `tarot.metrics`, which is pure Python for the same
...
wobble in a three-question bin as miscalibration. `tarot.metrics` already tracks
...
from tarot.metrics import Bin, ECEBinned
